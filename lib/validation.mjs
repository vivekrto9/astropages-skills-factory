...
function requireString(value, location, errors, { pattern, max = 1024, requireVisible = false } = {}) {
  if (typeof value !== 'string' || value.length === 0 || value.length > max) {
    errors.push(`${location} must be a non-empty string of at most ${max} characters`);
    return false;
  }
  if (/\p{C}/u.test(value)) {
    errors.push(`${location} must not contain Unicode control or format characters`);
    return false;
  }
  if (requireVisible && !/[\p{L}\p{N}\p{P}\p{S}]/u.test(value)) {
    errors.push(`${location} must contain at least one visible letter, number, punctuation mark, or symbol`);
    return false;
  }
...
function validateSkillCatalog(document, errors) {
  ...
  document.skills.forEach((skill, index) => {
    ...
    exactKeys(skill, ['id', 'version', 'description', 'activityLabel', 'compatibleAiRuntime', 'entry', 'resources', 'intents'], where, errors);
    ...
    requireString(skill?.activityLabel, `${where}.activityLabel`, errors, { max: 80, requireVisible: true });
...
async function validateSkillFiles(sourceRoot, skills, errors) {
  ...
  for (const skill of skills) {
    ...
    try {
      ...
      if (frontmatter && (typeof frontmatter.description !== 'string' || !frontmatter.description.startsWith('Use this skill when '))) {
        errors.push(`${skill.id}/${skill.entry} frontmatter description must start with "Use this skill when "`);
      }
