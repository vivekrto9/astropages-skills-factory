...
- Control Plane rejects normalized catalog alias collisions for the public picker, validates only explicit user-selected intents, and otherwise sends native mode with no inferred intents, skills, dependencies, or activation markers.
...
- The AI service may expose only its three compact AstrologyAPI discovery tools to a pinned v2 OpenHands runtime when the gateway is configured. The provider-adapter skill decides when they are relevant. Trusted AI deployment secret/env management exclusively supplies `ASTROLOGYAPI_MCP_ENABLED`, `ASTROLOGYAPI_MCP_URL`, and `ASTROLOGYAPI_MCP_KEY`. The AI proxy holds the upstream credential in service memory and adds `x-astrologyapi-key` outbound, never to OpenHands env, skills, payloads, telemetry, or Control Plane; this platform config is not Task 11.
...
- OpenHands SDK is pinned to 1.24; every pinned internal skill is available through native `invoke_skill` progressive disclosure. Explicit selections activate mandatory mappings, while all other invocation evidence comes only from real OpenHands observations.
- Client provides a grouped `@` picker, Skills button, `Selected by you` before execution, and `Added automatically` only after authoritative runtime invocation evidence.
...
## Architecture correction — 2026-07-22

The historical checkpoints below record what earlier iterations implemented and tested; their references to Control Plane inference, exact-boundary automatic routing, `KeywordTrigger` markers, and pre-run Client `Auto` chips are superseded and must not be used as current design guidance. The current contract is OpenHands-native selection from the full pinned skill catalog, optional explicit public selection, real `invoke_skill` evidence, and the compact AstrologyAPI discovery gateway. The three-slice files remain implementation-quality scenarios, not proof of activation precision or release evaluation evidence.
